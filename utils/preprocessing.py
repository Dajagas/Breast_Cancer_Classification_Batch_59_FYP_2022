...
class PreProcess:
    ...
    def apply_median_blur(self,image):
        blur_image = image.astype(np.uint8)
        blur_image = cv2.medianBlur(blur_image,3)
        return blur_image
    ...
    def run(self,img_path):
        ...
        median_blur_image = self.apply_median_blur(img)
        artiless_img = self.apply_artifact_removal(median_blur_image)
        complete_preprocess_image = self.apply_clahe(artiless_img)
